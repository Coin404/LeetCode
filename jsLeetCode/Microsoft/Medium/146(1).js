...
// 找到该点取出，头插
LinkedList.prototype.upHead = function (node) {
  const pre = node.pre;
...
LRUCache.prototype.get = function (key) {
  if (this.haxiLRU.has(key)) {
    ...
    return this.haxiLRU.get(key).value;
  } else {
    return -1;
  }
};
...
LRUCache.prototype.put = function (key, value) {
  if (this.link.length < this.length && !this.haxiLRU.has(key)) {
    ...
    this.haxiLRU.set(key, node);
    this.link.length++;
    this.link.headPush(node);
  } else if (this.haxiLRU.has(key)) {
    // 移动到头部
    this.link.upHead(this.haxiLRU.get(key));
    this.haxiLRU.get(key).value =value
